 ...
 * @author Leandro
 */
public class InterfaceAutor extends javax.swing.JDialog {

    public boolean alterar;

    public InterfaceAutor(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        listarAutores();
        setModal(true);
    ...
    public void listarAutores() {
        ...
        for (int i = 0; i < Autor.listaAutores.size(); i++) {
            dtm.addRow(new Object[]{
                Autor.listaAutores.get(i).getCodigo(),
                Autor.listaAutores.get(i).getNomeAutor()
            });
        }
    }
    
    public boolean validaCampos() {
        if (Functions.isEmpty(txtNomeAutor.getText())) {
            JOptionPane.showMessageDialog(rootPane, "POR FAVOR, INFORME O NOME DO AUTOR !", "Atenção", JOptionPane.WARNING_MESSAGE);
            txtNomeAutor.grabFocus();
            return false;
        }
        return true;
    }
    ...
    private void initComponents() {
        ...
        jPanel1.add(txtNomeAutor, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 110, 310, -1));
        ...
        jPanel1.add(btnSalvar, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 170, 169, 55));
        ...
        jPanel1.add(btnCancelar, new org.netbeans.lib.awtextra.AbsoluteConstraints(380, 170, 169, 55));
        jPanel1.add(jSeparator1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 160, 720, 10));
        ...
        if (jTableAutor.getColumnModel().getColumnCount() > 0) {
            jTableAutor.getColumnModel().getColumn(0).setMaxWidth(59);
        }
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(jTabPane, javax.swing.GroupLayout.PREFERRED_SIZE, 724, javax.swing.GroupLayout.PREFERRED_SIZE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(jTabPane, javax.swing.GroupLayout.PREFERRED_SIZE, 262, javax.swing.GroupLayout.PREFERRED_SIZE)
        );
    ...
    private void btnSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSalvarActionPerformed
        if (validaCampos() == false) {
            ...
            nomeAutor = Functions.spaceRemover(nomeAutor);

            if (alterar == false) {
                Autor autor = new Autor();
                autor.cadastrar(codigo, nomeAutor);
                Autor.listaAutores.add(autor);

                DefaultTableModel dtm = (DefaultTableModel) jTableAutor.getModel();
                int ultimaPos = Autor.listaAutores.size() - 1;

                dtm.addRow(new Object[]{
                    Autor.listaAutores.get(ultimaPos).getCodigo(),
                    Autor.listaAutores.get(ultimaPos).getNomeAutor()
                });
                JOptionPane.showMessageDialog(null, "REGISTRO SALVO COM SUCESSO !", "Aviso", JOptionPane.INFORMATION_MESSAGE);
                Autor.incrementaCodigo++;
                txtCodigo.setText(String.valueOf(Autor.incrementaCodigo));
                jTabPane.setEnabledAt(0, false);
                jTabPane.setEnabledAt(1, true);
            } else {
                alterar = false;

                Autor.listaAutores.get(jTableAutor.getSelectedRow()).setCodigo(codigo);
                Autor.listaAutores.get(jTableAutor.getSelectedRow()).setNomeAutor(nomeAutor);

                jTableAutor.setValueAt(Autor.listaAutores.get(jTableAutor.getSelectedRow()).getCodigo(), jTableAutor.getSelectedRow(), 0);
                jTableAutor.setValueAt(Autor.listaAutores.get(jTableAutor.getSelectedRow()).getNomeAutor(), jTableAutor.getSelectedRow(), 1);
    ...
    private void jTableAutorMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableAutorMouseReleased
        if (jTableAutor.getSelectedRow() != -1) {
            txtCodigo.setText(jTableAutor.getValueAt(jTableAutor.getSelectedRow(), 0).toString());
            txtNomeAutor.setText(jTableAutor.getValueAt(jTableAutor.getSelectedRow(), 1).toString());
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        DefaultTableModel dtmAutor = (DefaultTableModel) jTableAutor.getModel();
        boolean verificaVinculo = false;
        for (int i = 0; i < Livro.listaLivros.size(); i++) {
            if (jTableAutor.getValueAt(jTableAutor.getSelectedRow(), 0) == Livro.listaLivros.get(i).getAutor().getCodigo()) {
                verificaVinculo = true;
            }
        }

        if (verificaVinculo == true) {
            JOptionPane.showMessageDialog(null, "NÃO É POSSÍVEL EXCLUIR !\n HÁ LIVRO CADASTRO COM ESTE AUTOR !", "Atenção", JOptionPane.WARNING_MESSAGE);
        } else {
            int opcao = JOptionPane.showConfirmDialog(rootPane, "TEM CERTEZA QUE DESEJA EXCLUIR O AUTOR SELECIONADO?", "Atenção!", JOptionPane.YES_NO_OPTION);
            int opcaoSelecionada = jTableAutor.getSelectedRow();
            if (opcao == JOptionPane.YES_OPTION) {
                dtmAutor.removeRow(opcaoSelecionada);
                Autor.listaAutores.remove(opcaoSelecionada);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(InterfaceAutor.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                InterfaceAutor dialog = new InterfaceAutor(new javax.swing.JFrame(), true);
